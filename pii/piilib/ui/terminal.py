...
class PiiTerminal(PiiExtendedPlainTextEdit):
    ...
    def __init__(self):
        ...
        self.write_user_prompt(pformat=PiiTextFormats().BOLD_RED)
    ...
    def get_completer(self, matching_list):
        return completer.PiiAutoCompleter(matching_list, widget=self, font=self.text_editor_font,
                                          on_completion=self.replace_text_under_cursor)
    ...
    def write_user_prompt(self, pformat=None):
    ...
    def run_command(self, multiline):
        ...
        if not multiline or (multiline and not command_text):
            with self.use_multiline_prompt(prompt=self.RESULT_PROMPT):
                self.write_user_prompt(pformat=PiiTextFormats().BOLD_GREEN)
                self.insert_line_break(1)
